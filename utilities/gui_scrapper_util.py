...
verse_name_list = utils.dql_retrieve_data(database_file, const.SQL__GET_CONCLUDING_LIST)
for verse_name in verse_name_list:
    print(f'Converting shloka from {verse_name}')
    verses_list = utils.dql_retrieve_data_custominput(database_file, select_script, verse_name)
    for shloka in verses_list:
        try:
            ...
            latin_output.clear()
            utils.dml_execute_script(database_file, insert_statement)
